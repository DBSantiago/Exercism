 ...
 */
 export function totalBirdCount(birdsPerDay) {
  let totalBirdsCounted = 0;
  for(let i = 0; i < birdsPerDay.length; i++){
    totalBirdsCounted += birdsPerDay[i];
  }
  return totalBirdsCounted;
}
...
export function birdsInWeek(birdsPerDay, week) {
  let weekStartingIndex = (week * 7 - 7);
  let weekEndingIndex = weekStartingIndex + 7;
  let totalBirdsCountedInWeek = 0;
  for(let i = weekStartingIndex; i < weekEndingIndex; i++){
    totalBirdsCountedInWeek += birdsPerDay[i];
  }
  return totalBirdsCountedInWeek;
}
